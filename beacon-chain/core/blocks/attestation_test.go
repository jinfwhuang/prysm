...
func TestProcessAggregatedAttestation_OverlappingBits(t *testing.T) {
	...
	for i, indice := range attestingIndices1 {
		sb, err := core.ComputeDomainAndSign(beaconState, 0, att1.Data, params.BeaconConfig().DomainBeaconAttester, privKeys[indice])
		require.NoError(t, err)
	...
	for i, indice := range attestingIndices2 {
		sb, err := core.ComputeDomainAndSign(beaconState, 0, att2.Data, params.BeaconConfig().DomainBeaconAttester, privKeys[indice])
		require.NoError(t, err)
		sig, err := bls.SignatureFromBytes(sb)
		require.NoError(t, err)
		sigs[i] = sig
...
func TestVerifyIndexedAttestation_OK(t *testing.T) {
	...
	for _, tt := range tests {
		...
		for _, idx := range tt.attestation.AttestingIndices {
			sb, err := core.ComputeDomainAndSign(state, tt.attestation.Data.Target.Epoch, tt.attestation.Data, params.BeaconConfig().DomainBeaconAttester, keys[idx])
			require.NoError(t, err)
...
func TestVerifyAttestations_HandlesPlannedFork(t *testing.T) {
	...
	prevDomain, err := core.Domain(st.Fork(), st.Fork().Epoch-1, params.BeaconConfig().DomainBeaconAttester, st.GenesisValidatorRoot())
	require.NoError(t, err)
	root, err := core.ComputeSigningRoot(att1.Data, prevDomain)
	require.NoError(t, err)
	...
	currDomain, err := core.Domain(st.Fork(), st.Fork().Epoch, params.BeaconConfig().DomainBeaconAttester, st.GenesisValidatorRoot())
	require.NoError(t, err)
	root, err = core.ComputeSigningRoot(att2.Data, currDomain)
	require.NoError(t, err)
	sigs = nil
	for i, u := range comm2 {
...
func TestRetrieveAttestationSignatureSet_VerifiesMultipleAttestations(t *testing.T) {
	...
	domain, err := core.Domain(st.Fork(), st.Fork().Epoch, params.BeaconConfig().DomainBeaconAttester, st.GenesisValidatorRoot())
	require.NoError(t, err)
	root, err := core.ComputeSigningRoot(att1.Data, domain)
	require.NoError(t, err)
	var sigs []bls.Signature
	for i, u := range comm1 {
		att1.AggregationBits.SetBitAt(uint64(i), true)
	...
	root, err = core.ComputeSigningRoot(att2.Data, domain)
	require.NoError(t, err)
	sigs = nil
	for i, u := range comm2 {
		att2.AggregationBits.SetBitAt(uint64(i), true)
