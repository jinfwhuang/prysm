...
func TestAttestToBlockHead_AttestsCorrectly(t *testing.T) {
	...
	root, err := core.ComputeSigningRoot(expectedAttestation.Data, make([]byte, 32))
...
func TestSignAttestation(t *testing.T) {
	...
	attesterDomain, err := core.Domain(wantedFork, 0, params.BeaconConfig().DomainBeaconAttester, genesisValidatorRoot[:])
	require.NoError(t, err)
