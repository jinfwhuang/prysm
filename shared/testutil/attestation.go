...
func GenerateAttestations(
	...
	domain, err := core2.Domain(bState.Fork(), currentEpoch, params.BeaconConfig().DomainBeaconAttester, bState.GenesisValidatorRoot())
	if err != nil {
		return nil, err
	}
	for c := types.CommitteeIndex(0); uint64(c) < committeesPerSlot && uint64(c) < numToGen; c++ {
		...
		dataRoot, err := core2.ComputeSigningRoot(attData, domain)
		if err != nil {
			return nil, err
		}
