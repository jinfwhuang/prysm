...
// Hash: 436a86e2fe290650f73c487d06a3531ba24629eba1ebf3b4300188bada64bf8a
package eth
...
func (l *LightClientUpdate) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (7) 'ForkVersion'
	if len(l.ForkVersion) != 4 {
		err = ssz.ErrBytesLength
		return
	}
	dst = append(dst, l.ForkVersion...)
...
func (l *LightClientUpdate) UnmarshalSSZ(buf []byte) error {
	...
	if err = l.FinalityHeader.UnmarshalSSZ(buf[24896:25008]); err != nil {
		return err
	}
	...
	if cap(l.SyncCommitteeBits) == 0 {
		l.SyncCommitteeBits = make([]byte, 0, len(buf[25200:25264]))
	}
	l.SyncCommitteeBits = append(l.SyncCommitteeBits, buf[25200:25264]...)

	// Field (6) 'SyncCommitteeSignature'
	if cap(l.SyncCommitteeSignature) == 0 {
		l.SyncCommitteeSignature = make([]byte, 0, len(buf[25264:25360]))
	}
	l.SyncCommitteeSignature = append(l.SyncCommitteeSignature, buf[25264:25360]...)

	// Field (7) 'ForkVersion'
	if cap(l.ForkVersion) == 0 {
		l.ForkVersion = make([]byte, 0, len(buf[25360:25364]))
	}
	l.ForkVersion = append(l.ForkVersion, buf[25360:25364]...)
...
func (l *LightClientUpdate) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	...
	hh.PutBytes(l.ForkVersion)

	hh.Merkleize(indx)
	return
}

// MarshalSSZ ssz marshals the SkipSyncUpdate object
func (s *SkipSyncUpdate) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(s)
}

// MarshalSSZTo ssz marshals the SkipSyncUpdate object to a target array
func (s *SkipSyncUpdate) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf

	// Field (0) 'AttestedHeader'
	if s.AttestedHeader == nil {
		s.AttestedHeader = new(BeaconBlockHeader)
	}
	if dst, err = s.AttestedHeader.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (1) 'CurrentSyncCommittee'
	if s.CurrentSyncCommittee == nil {
		s.CurrentSyncCommittee = new(SyncCommittee)
	}
	if dst, err = s.CurrentSyncCommittee.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (2) 'CurrentSyncCommitteeBranch'
	if len(s.CurrentSyncCommitteeBranch) != 5 {
		err = ssz.ErrVectorLength
		return
	}
	for ii := 0; ii < 5; ii++ {
		if len(s.CurrentSyncCommitteeBranch[ii]) != 32 {
			err = ssz.ErrBytesLength
			return
		}
		dst = append(dst, s.CurrentSyncCommitteeBranch[ii]...)
	}

	// Field (3) 'NextSyncCommittee'
	if s.NextSyncCommittee == nil {
		s.NextSyncCommittee = new(SyncCommittee)
	}
	if dst, err = s.NextSyncCommittee.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (4) 'NextSyncCommitteeBranch'
	if len(s.NextSyncCommitteeBranch) != 5 {
		err = ssz.ErrVectorLength
		return
	}
	for ii := 0; ii < 5; ii++ {
		if len(s.NextSyncCommitteeBranch[ii]) != 32 {
			err = ssz.ErrBytesLength
			return
		}
		dst = append(dst, s.NextSyncCommitteeBranch[ii]...)
	}

	// Field (5) 'FinalityHeader'
	if s.FinalityHeader == nil {
		s.FinalityHeader = new(BeaconBlockHeader)
	}
	if dst, err = s.FinalityHeader.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (6) 'FinalityBranch'
	if len(s.FinalityBranch) != 6 {
		err = ssz.ErrVectorLength
		return
	}
	for ii := 0; ii < 6; ii++ {
		if len(s.FinalityBranch[ii]) != 32 {
			err = ssz.ErrBytesLength
			return
		}
		dst = append(dst, s.FinalityBranch[ii]...)
	}

	// Field (7) 'SyncCommitteeBits'
	if len(s.SyncCommitteeBits) != 64 {
		err = ssz.ErrBytesLength
		return
	}
	...
	// Field (8) 'SyncCommitteeSignature'
	if len(s.SyncCommitteeSignature) != 96 {
		err = ssz.ErrBytesLength
		return
	}
...
// UnmarshalSSZ ssz unmarshals the SkipSyncUpdate object
func (s *SkipSyncUpdate) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size != 50148 {
		return ssz.ErrSize
	...
	if err = s.AttestedHeader.UnmarshalSSZ(buf[0:112]); err != nil {
		return err
	}
	...
	if err = s.CurrentSyncCommittee.UnmarshalSSZ(buf[112:24736]); err != nil {
		return err
	}

	// Field (2) 'CurrentSyncCommitteeBranch'
	s.CurrentSyncCommitteeBranch = make([][]byte, 5)
	for ii := 0; ii < 5; ii++ {
		if cap(s.CurrentSyncCommitteeBranch[ii]) == 0 {
			s.CurrentSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[24736:24896][ii*32:(ii+1)*32]))
		}
		s.CurrentSyncCommitteeBranch[ii] = append(s.CurrentSyncCommitteeBranch[ii], buf[24736:24896][ii*32:(ii+1)*32]...)
	}
	...
	if err = s.NextSyncCommittee.UnmarshalSSZ(buf[24896:49520]); err != nil {
		return err
	}

	// Field (4) 'NextSyncCommitteeBranch'
	s.NextSyncCommitteeBranch = make([][]byte, 5)
	for ii := 0; ii < 5; ii++ {
		if cap(s.NextSyncCommitteeBranch[ii]) == 0 {
			s.NextSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[49520:49680][ii*32:(ii+1)*32]))
		}
		s.NextSyncCommitteeBranch[ii] = append(s.NextSyncCommitteeBranch[ii], buf[49520:49680][ii*32:(ii+1)*32]...)
	}
	...
	if err = s.FinalityHeader.UnmarshalSSZ(buf[49680:49792]); err != nil {
		return err
	}

	// Field (6) 'FinalityBranch'
	s.FinalityBranch = make([][]byte, 6)
	for ii := 0; ii < 6; ii++ {
		if cap(s.FinalityBranch[ii]) == 0 {
			s.FinalityBranch[ii] = make([]byte, 0, len(buf[49792:49984][ii*32:(ii+1)*32]))
		}
		s.FinalityBranch[ii] = append(s.FinalityBranch[ii], buf[49792:49984][ii*32:(ii+1)*32]...)
	}

	// Field (7) 'SyncCommitteeBits'
	if cap(s.SyncCommitteeBits) == 0 {
		s.SyncCommitteeBits = make([]byte, 0, len(buf[49984:50048]))
	}
	s.SyncCommitteeBits = append(s.SyncCommitteeBits, buf[49984:50048]...)

	// Field (8) 'SyncCommitteeSignature'
	if cap(s.SyncCommitteeSignature) == 0 {
		s.SyncCommitteeSignature = make([]byte, 0, len(buf[50048:50144]))
	}
	s.SyncCommitteeSignature = append(s.SyncCommitteeSignature, buf[50048:50144]...)

	// Field (9) 'ForkVersion'
	if cap(s.ForkVersion) == 0 {
		s.ForkVersion = make([]byte, 0, len(buf[50144:50148]))
	}
	s.ForkVersion = append(s.ForkVersion, buf[50144:50148]...)

	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the SkipSyncUpdate object
func (s *SkipSyncUpdate) SizeSSZ() (size int) {
	size = 50148
	return
}

// HashTreeRoot ssz hashes the SkipSyncUpdate object
func (s *SkipSyncUpdate) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(s)
}

// HashTreeRootWith ssz hashes the SkipSyncUpdate object with a hasher
func (s *SkipSyncUpdate) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()

	// Field (0) 'AttestedHeader'
	if err = s.AttestedHeader.HashTreeRootWith(hh); err != nil {
		return
	}
	...
	// Field (2) 'CurrentSyncCommitteeBranch'
	{
		...
		subIndx := hh.Index()
		for _, i := range s.CurrentSyncCommitteeBranch {
			if len(i) != 32 {
				err = ssz.ErrBytesLength
				return
			}
	...
	// Field (4) 'NextSyncCommitteeBranch'
	{
		...
		subIndx := hh.Index()
		for _, i := range s.NextSyncCommitteeBranch {
			if len(i) != 32 {
				err = ssz.ErrBytesLength
				return
			}
	...
	// Field (5) 'FinalityHeader'
	if err = s.FinalityHeader.HashTreeRootWith(hh); err != nil {
		return
	}
	...
	// Field (6) 'FinalityBranch'
	{
		...
		subIndx := hh.Index()
		for _, i := range s.FinalityBranch {
			if len(i) != 32 {
				err = ssz.ErrBytesLength
				return
			}
