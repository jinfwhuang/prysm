...
// Hash: f71de9cb774c58daf2d6425d8f35c8cba8e5d4ab82e2f368b3550040c9335bef
package eth
...
func (s *SkipSyncUpdate) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (9) 'CurrentSyncCommitteeBranch'
...
func (s *SkipSyncUpdate) UnmarshalSSZ(buf []byte) error {
	...
	if err = s.NextSyncCommittee.UnmarshalSSZ(buf[112:24736]); err != nil {
		return err
	}
	...
	for ii := 0; ii < 5; ii++ {
		if cap(s.NextSyncCommitteeBranch[ii]) == 0 {
			s.NextSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[24736:24896][ii*32:(ii+1)*32]))
		}
		s.NextSyncCommitteeBranch[ii] = append(s.NextSyncCommitteeBranch[ii], buf[24736:24896][ii*32:(ii+1)*32]...)
	}
	...
	if err = s.FinalityHeader.UnmarshalSSZ(buf[24896:25008]); err != nil {
		return err
	}
	...
	for ii := 0; ii < 6; ii++ {
		if cap(s.FinalityBranch[ii]) == 0 {
			s.FinalityBranch[ii] = make([]byte, 0, len(buf[25008:25200][ii*32:(ii+1)*32]))
		}
		s.FinalityBranch[ii] = append(s.FinalityBranch[ii], buf[25008:25200][ii*32:(ii+1)*32]...)
	}

	// Field (5) 'SyncCommitteeBits'
	if cap(s.SyncCommitteeBits) == 0 {
		s.SyncCommitteeBits = make([]byte, 0, len(buf[25200:25264]))
	}
	s.SyncCommitteeBits = append(s.SyncCommitteeBits, buf[25200:25264]...)

	// Field (6) 'SyncCommitteeSignature'
	if cap(s.SyncCommitteeSignature) == 0 {
		s.SyncCommitteeSignature = make([]byte, 0, len(buf[25264:25360]))
	}
	s.SyncCommitteeSignature = append(s.SyncCommitteeSignature, buf[25264:25360]...)

	// Field (7) 'ForkVersion'
	if cap(s.ForkVersion) == 0 {
		s.ForkVersion = make([]byte, 0, len(buf[25360:25364]))
	}
	s.ForkVersion = append(s.ForkVersion, buf[25360:25364]...)

	// Field (8) 'CurrentSyncCommittee'
	if s.CurrentSyncCommittee == nil {
		s.CurrentSyncCommittee = new(SyncCommittee)
	}
	if err = s.CurrentSyncCommittee.UnmarshalSSZ(buf[25364:49988]); err != nil {
		return err
	}
	...
	for ii := 0; ii < 5; ii++ {
		if cap(s.CurrentSyncCommitteeBranch[ii]) == 0 {
			s.CurrentSyncCommitteeBranch[ii] = make([]byte, 0, len(buf[49988:50148][ii*32:(ii+1)*32]))
		}
		s.CurrentSyncCommitteeBranch[ii] = append(s.CurrentSyncCommitteeBranch[ii], buf[49988:50148][ii*32:(ii+1)*32]...)
	}
